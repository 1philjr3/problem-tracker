...
import { cloudDataService, type LocalUser, type SeasonSettings } from '../../services/cloudDataService';
...
const LeaderboardPage: React.FC = () => {
  ...
  const handleFinishSeason = async () => {
    ...
    if (window.confirm('Завершить сезон? Это покажет финальные результаты всем пользователям!')) {
      try {
        const { report } = await cloudDataService.finishSeason(currentUser.uid, currentUser.email || '');
  ...
  const fixUserNames = async () => {
    try {
      setLoading(true);
      await cloudDataService.fixUserNames();
      ...
      <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row gap-3 sm:gap-4">
        ...
        {isAdmin ? (
          <>
            ...
            <button
              onClick={() => cloudDataService.exportData()}
